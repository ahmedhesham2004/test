...
export default function GetBySpecialtie() {
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        ...
        if (!res.ok) throw new Error("فشل في جلب قائمة التخصصات");
        const result = await res.json();
        setDoctors(result);
      } catch (err) {
        console.error(err);
        setError("فشل في جلب بيانات التخصصات");
      }
    };
  ...
  const handlePrint = () => {
    ...
    if (data && data.transactions && data.transactions.length > 0) {
      ...
      printContent += `
        <div>
          <p><strong>اسم التخصص:</strong> ${specialty ? specialty.name : 'غير معروف'}</p>
              ...
              <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-4 rounded-t-2xl flex justify-between items-center">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-xl font-bold text-white">
                    بحث عن المعاملات المالية للتخصصات 
                  </h2>
                </div>
                <button
                  onClick={closeModal}
